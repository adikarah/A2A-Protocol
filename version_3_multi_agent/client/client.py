...
from version_3_multi_agent.models.request import SendTaskRequest, GetTaskRequest  # Removed CancelTaskRequest

# Base request format for JSON-RPC 2.0
from version_3_multi_agent.models.json_rpc import JSONRPCRequest

# Models for task results and agent identity
from version_3_multi_agent.models.task import Task, TaskSendParams
...
class A2AClient:
    ...
    async def send_task(self, payload: dict[str, Any]) -> Task:
        ...
        request = SendTaskRequest(
            ...
            params=(
                payload
                if isinstance(payload, TaskSendParams)
                else TaskSendParams(**payload)
            )  # Proper model wrapping
        )
    ...
    async def _send_request(self, request: JSONRPCRequest) -> dict[str, Any]:
        async with httpx.AsyncClient(
                timeout=httpx.Timeout(120.0)  # timeout
        ) as client:
            try:
