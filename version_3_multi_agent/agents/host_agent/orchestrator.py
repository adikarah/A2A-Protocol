...
# -----------------------------------------------------------------------------
from version_3_multi_agent.agents.host_agent.agent_connect import AgentConnector
...
class OrchestratorAgent:
    ...
    def _build_agent(self) -> LlmAgent:
        ...
        """
        return LlmAgent(
            model="gemini-3-flash-preview",    # Specify Gemini model version
    ...
    def _root_instruction(self, context: ReadonlyContext) -> str:
        ...
        return (
            ...
            "Your job is to route user queries to the correct agent.\n\n"

            "Available agents:\n"
            f"{agent_list}\n\n"

            "Routing Rules:\n"
            "- Use TripPlannerAgent for travel-related queries\n"
            "- Use ExplainAgent for concept explanations\n"
            "- If query contains multiple intents, call multiple agents\n"
            "- Combine responses clearly\n"
            "- DO NOT answer yourself\n\n"

            "You have tools:\n"
            "1. list_agents()\n"
            "2. delegate_task(agent_name, message)\n"
        )
    ...
    #     logger.info(f"Delegating → {agent_name} | message: {message}")
    ...
    async def _delegate_task(
        ...
        if child_task.history and len(child_task.history) > 1:
            result = child_task.history[-1].parts[0].text

            # IMPORTANT: return directly (NO extra LLM processing)
            return result
...
class OrchestratorTaskManager(InMemoryTaskManager):
    ...
    async def on_send_task(self, request: SendTaskRequest) -> SendTaskResponse:
        ...
        async with self.lock:
            ...
            task.history.append(reply.model_dump())
