...
from version_3_multi_agent.utilities.discovery import DiscoveryClient
...
from version_3_multi_agent.models.agent import AgentCard, AgentCapabilities, AgentSkill
# Orchestrator implementation and its task manager
from version_3_multi_agent.agents.host_agent.orchestrator import (
    OrchestratorAgent,
...
def main(host: str, port: int, registry: str):
    ...
    skill = AgentSkill(
        ...
        description=(
            "Routes user requests to specialized agents like trip planning "
            "and knowledge explanation based on intent"
        ),
        tags=["routing", "orchestration"],       # Keywords to aid discovery
        examples=[
            "Plan a trip to Goa",
            "Explain microservices",
            "Plan a Goa trip and explain microservices"
        ]
    )
