...
from version_2_adk_agent.server.server import A2AServer

# Models for describing agent capabilities and metadata
from version_2_adk_agent.models.agent import AgentCard, AgentCapabilities, AgentSkill
...
def main(host, port):
    ...
    skill = AgentSkill(
        id="explain",                                 # Unique skill ID
        name="Explain Tool",                          # Human-friendly name
        description="Explains concepts, answers questions, and provides detailed responses",    # What the skill does
        tags=["explain", "qa", "knowledge"],                                  # Optional tags for searching
        examples=["Explain microservices",
        "What is A2A protocol?",
        "Explain JVM memory model"]  # Example queries
    )

    # Create an agent card describing this agent’s identity and metadata
    agent_card = AgentCard(
        name="ExplainAgent",                               # Name of the agent
        description="This agent explains concepts and answers questions using LLM.",  # Description
        url=f"http://{host}:{port}/",                       # The public URL where this agent lives
        version="1.0.0",                                    # Version number
        defaultInputModes=ExplainAgent.SUPPORTED_CONTENT_TYPES,  # Input types this agent supports
        defaultOutputModes=ExplainAgent.SUPPORTED_CONTENT_TYPES, # Output types it produces
    ...
    server = A2AServer(
        ...
        task_manager=AgentTaskManager(agent=ExplainAgent())
    )
